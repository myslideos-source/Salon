...
**Status: 🟡 teilweise umgesetzt** im Sonderauftrag „Standorte, Team, Ressourcen und Verfügbarkeit" oben — dort für `employees`, `employee_working_hours`, `resources`, `resource_working_hours`, `service_resources`, `locations`, `business_hours`, `business_hour_exceptions` per `has_permission()`-gesteuerten Policies umgesetzt, inkl. neuer Formulare in `/app/team` und `/app/availability`. Offen bleibt ausdrücklich `services` (Terminarten/Leistungen) — das ist ein eigener Konzeptabschnitt und war nicht Teil dieses Auftrags; `/app/services` bleibt bis dahin lesend.
