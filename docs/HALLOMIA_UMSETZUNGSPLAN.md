...
**Status: 🟡 teilweise umgesetzt** im Sonderauftrag „Branchenvorlagen und Onboarding-Grundlage" (Schritte 1–3) und, seither weiter fortgeführt, im Sonderauftrag „Standorte, Team, Ressourcen und Verfügbarkeit" oben (Schritte 4–6: Standort, Öffnungszeiten, Team & Ressourcen — jeweils dieselben Manager-Komponenten wie die zugehörigen Verwaltungsseiten, keine Duplikate). `/app/onboarding` deckt damit inzwischen sechs der zwölf Konzept-Schritte interaktiv ab (`LAST_INTERACTIVE_STEP = 6`). Offen bleiben die Schritte 7–12 (Terminarten, Meine Mia, häufige Fragen, Kalender testen, Telefonnummer verbinden, Mia aktivieren) — sie hängen jeweils an ihrer eigenen Fachbereichs-Phase (7–13) und werden dort ergänzt, sobald diese beauftragt werden.
