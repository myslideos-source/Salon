...
import { updateBusinessHoursSelfAction } from "@/lib/actions/availability-settings";
...
export function OpeningHoursForm({ salonId, rows, canManage }: { salonId: string; rows: Row[]; canManage: boolean }) {
  ...
  return (
    <form
      action={async (fd) => {
        await updateBusinessHoursSelfAction(salonId, fd);
        setSaved(true);
      ...
      {rows.map((row) => (
        <div key={row.weekday} className="flex flex-wrap items-center gap-3">
          ...
          <Input
            ...
            disabled={closed[row.weekday] || !canManage}
            className="w-32"
          />
