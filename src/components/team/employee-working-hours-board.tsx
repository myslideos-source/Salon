...
import {
  addEmployeeWorkingHourSelfAction,
  deleteEmployeeWorkingHourSelfAction,
  type ActionState,
...
export function EmployeeWorkingHoursBoard({
  salonId,
  employees,
  workingHours,
  canManage,
  ...
  const action = addEmployeeWorkingHourSelfAction.bind(null, salonId);
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {dayBlocks.map((b) => (
                        <span key={b.id} className="inline-flex items-center gap-1 rounded-full bg-sand px-2 py-1 text-xs text-ink-soft">
                          ...
                          {canManage && (
                            <button onClick={() => deleteEmployeeWorkingHourSelfAction(salonId, b.id)} aria-label="Entfernen">
